...
export default function InputTahsinPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (error) throw error;
        setNotification({ show: true, message: "Data tahsin berhasil disimpan!", type: 'success' });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await new Promise(resolve => setTimeout(resolve, 800)); // Efek loading buatan
      const { error } = await supabase.from("laporan_tahsin_tahfidz").delete().eq("id", id);
      if (error) throw error;
    ...
    <DashboardLayout userRole="guru">
      <div className="mb-8">
        <h1 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tight mb-2">
          Input Tahsin & Tahfidz
        </h1>
        <p className="text-sm text-gray-500 font-medium">
          Pilih siswa dan masukkan laporan hafalan Al-Qur'an harian mereka.
        </p>
      </div>

      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 border-t-4 border-[#3b82f6] max-w-3xl">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 bg-[#3b82f6]/10 text-[#3b82f6] rounded-xl flex items-center justify-center">
            <BookOpen size={24} />
          </div>
          <h2 className="text-2xl font-black text-gray-900">Form Laporan Tahsin</h2>
        </div>
        
        {loading ? (
           <div className="flex justify-center items-center h-32">
             <div className="animate-spin rounded-full h-8 w-8 border-t-4 border-b-4 border-[#3b82f6]"></div>
           </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  className="w-full px-5 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#3b82f6] focus:border-transparent transition-all font-medium appearance-none bg-white"
                  required
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-5 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#3b82f6] focus:border-transparent transition-all font-medium bg-white"
                  required
                />
              </div>
              ...
              <Input
                label="Catatan Makhraj (Opsional)"
                placeholder="Contoh: Perbaiki huruf dzal / ro"
                value={formData.makhraj}
                onChange={(e) => setFormData({ ...formData, makhraj: e.target.value })}
              />

              <Input
                label="Surah Muroja'ah (Opsional)"
                placeholder="Contoh: An-Nas"
            ...
            <div className="flex gap-4">
              <Button type="submit" disabled={isSubmitting} className="flex-1 font-bold py-4 rounded-xl flex items-center justify-center gap-2 bg-[#3b82f6] hover:bg-[#2563eb] text-white shadow-lg disabled:opacity-70 disabled:cursor-not-allowed transition-all">
                {isSubmitting ? (
                  <><Loader2 size={20} className="animate-spin" /> Memproses...</>
                ) : (
                  <><Save size={20} /> {editingId ? "Update Laporan" : "Simpan Laporan Tahsin"}</>
                )}
              </Button>
              {editingId && (
                <Button 
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 border-t-4 border-gray-200 mt-8 max-w-5xl">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h2 className="text-xl font-black text-gray-900">
              Riwayat Tahsin & Tahfidz: <span className="text-[#3b82f6]">{students.find(s => s.id === formData.student_id)?.nama_lengkap || "Siswa"}</span>
            </h2>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50/50 text-xs uppercase font-bold text-gray-500 tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-4">Surah Baru</th>
                ...
                {history.length > 0 ? (
                  history.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-bold text-[#3b82f6]">{item.nama_surah}</td>
                      <td className="px-6 py-4 text-gray-600">{item.ayat}</td>
                      <td className="px-6 py-4 text-gray-600 font-arabic text-lg">{item.makhraj || "-"}</td>
                      <td className="px-6 py-4 text-gray-800 font-medium">{item.murojaah || "-"}</td>
